function [P] = ap_min_degree_noabsorb(Amtx)
    n = size(Amtx,1);
    ...
    while k <= n - length(dense_rows)
        ...
        for i = reshape(Lp, 1, [])
            ...
            % Ablation: Element absorption DISABLED
            Ei_arr = reshape(E{i}, 1, []);
            Ei_arr(Ei_arr == p) = []; % Still need to remove p itself
            
            % Keep ALL elements, even redundant ones
            new_Ei = Ei_arr; 
            
            % Calculate sum_we using all elements
