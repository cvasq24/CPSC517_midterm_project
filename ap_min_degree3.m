function [P] = ap_min_degree3(Amtx)
    n = size(Amtx,1);
    ...
    dense_threshold = max(16, 10 * sqrt(n));
    dense_rows = reshape(find(d > dense_threshold), 1, []);
    for i = dense_rows
        nv(i) = 0;
    end
    ...
    while k <= n - length(dense_rows)
        ...
        % elements. 
        Lp = A{p};
        for e = reshape(E{p}, 1, []), Lp = [Lp, Le{e}]; end
        Lp = unique(Lp);
        Lp = Lp(nv(Lp) > 0); % Only keep 'non-dead' nodes
        Lp(Lp == p) = []; % remove redundancy
        Le{p} = Lp; % update our Le cell
        ...
        for i = reshape(Lp, 1, [])
            ...
            % don't worry about that because previously we scanned all
            % elements in Ei 
            new_Ei = []; 
            sum_we = 0;
            for e = reshape(E{i}, 1, [])
